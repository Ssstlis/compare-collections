  ...
  "FieldComparator" - {
    ...
    "numericDiff" - {
      ...
      "computes absolute difference for two doubles (v2 > v1)" in {
        FieldComparator.numericDiff(
          Some(new BsonDouble(100.0)), Some(new BsonDouble(160.0)), 0
        ) shouldBe 60.0
      }

      "result is non-negative even when v1 > v2" in {
        FieldComparator.numericDiff(
          Some(new BsonDouble(200.0)), Some(new BsonDouble(150.0)), 0
        ) shouldBe 50.0
      }

      "computes difference between BsonInt32 and BsonDouble" in {
        FieldComparator.numericDiff(
          Some(new BsonInt32(10)), Some(new BsonDouble(13.0)), 0
        ) shouldBe 3.0
      }

      "returns 0.0 when one value is a non-numeric BsonString" in {
        FieldComparator.numericDiff(
          Some(new BsonString("x")), Some(new BsonDouble(5.0)), 0
        ) shouldBe 0.0
      }
      ...
      "rounds the result to the given precision" in {
        ...
        FieldComparator.numericDiff(
          Some(new BsonDouble(0.0)), Some(new BsonDouble(1.456)), 2
        ) shouldBe 1.46
      }

      "returns 0.0 for equal values" in {
        FieldComparator.numericDiff(
          Some(new BsonInt64(42L)), Some(new BsonInt64(42L)), 0
        ) shouldBe 0.0
      }
    }
  }
